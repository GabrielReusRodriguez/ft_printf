...
/*   Updated: 2024/01/30 01:05:34 by greus-ro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_print_formated_data(va_list argp, t_format *format)
{
	char	*str_formated;
	...
	if (format->c_data_type == 'c')
		str_formated = ft_printf_char(argp);
	*/
	if (format->c_data_type == '%')
		str_formated = ft_printf_percent();
...
{	
	...
	*i = *i + format->len;
	...
	if (format->c_data_type != 'c')
	{
		str_formated = ft_print_formated_data(argp, format);
